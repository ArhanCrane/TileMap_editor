...
grid::grid() {
	...
	for (size_t i{}; i < width; i++) {
		grid_map.push_back(std::vector<tile>(height, tile(sf::Vector2f(50.f, 50.f))));
	}

	for (size_t i{}; i < grid_map.size(); i++) {
		for (size_t j{}; j < grid_map[i].size(); j++) {
			grid_map[i][j].shape.setPosition({ 250.f + (50.f * i), 50.f + (50.f * j) });
		}
	}
...
void grid::thecolor(float x, float y, sf::Texture* texture) {
	if (texture == nullptr) {
		return;
	}
	...
	if ((cursorPosX <= width - 1 && cursorPosX >= 0) && (cursorPosY <= height - 1 && cursorPosY >= 0)) {		
		grid_map[cursorPosX][cursorPosY].shape.setTexture(texture);
		grid_map[cursorPosX][cursorPosY].shape.setFillColor(sf::Color::White);		
	}
	else {
		return;
	}
